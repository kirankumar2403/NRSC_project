...
class CloudShadowDetector:
    ...
    def load_image(self):
        ...
        band1 = self.src.read(1)
        print(f"[DEBUG] Input BAND1 stats: min={band1.min()}, max={band1.max()}, mean={band1.mean()}, unique={np.unique(band1)}")
    ...
    def save_results(self, output_dir):
        ...
        # Print mask statistics before saving
        print(f"[DEBUG] Mask stats: min={self.combined_mask.min()}, max={self.combined_mask.max()}, unique={np.unique(self.combined_mask)}")
